...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (LINKEDIN_CREDENTIALS.accessToken === 'YOUR_ACCESS_TOKEN') {
    return res.status(400).json({
      error: 'LinkedIn credentials not configured. Please provide your LinkedIn API credentials.'
    });
  }

  try {
    const { images, caption } = req.body as { images: string[]; caption: string };
    ...
    // Upload images (LinkedIn allows up to 20 images per post)
    ...
    if (imageAssets.length === 0) {
      return res.status(500).json({ error: 'Failed to upload any images to LinkedIn' });
    }

    // Create the post
    const postId = await createPost(personUrn, caption, imageAssets);
    ...
    return res.status(500).json({
      error: errorMessage,
      details: errorMessage, // Include full error for debugging
    });
  }
}
